...
const MAX_PAGES_PER_FUNCTION_CYCLE = parseInt(process.env.MAX_PAGES_PER_FUNCTION_CYCLE || '10'); // Limit pages fetched per function per cycle to avoid huge batches
...
function parseTransactionAndCollectMappingUpdates(tx: AleoTransaction, transition: AleoTransition, funcConfig: FunctionConfig, programConfig: ProgramConfig): { baseTxData: object; funcSpecificData: object; mappingUpdateCandidates: MappingUpdateCandidate[]; } {
        ...
        for (const trigger of funcConfig.triggersMappingUpdates) {
            ...
            if (key !== undefined) {
                const candidate: MappingUpdateCandidate = {
                    ...
                    key: JS2Leo(String(key), trigger.aleoType.kind === 'primitive' ? trigger.aleoType.type : undefined),
    ...
    if (tx.finalize && tx.finalizedAt) {
        for (const finalizeEntry of tx.finalize) {
            const matchingMappingConfig = programConfig.mappings?.find(m => m.name === finalizeEntry.mapping_id.split('/').pop());
      ...
      while (hasMoreTransactionsForFunction && pagesFetchedInThisFunctionCycle < MAX_PAGES_PER_FUNCTION_CYCLE) {
        ...
        try {
          ...
          transactionsBatch = (response || []).filter(tx =>
            ...
            tx.transaction?.execution?.transitions?.some(
              transition =>
            transition?.program === programConfig.programId &&
            transition?.function === funcConfig.name
            )
          );
    ...
    for (const tx of transactionsCollectedInThisCycle) {
        ...
        for (const transition of transitions) {
            ...
            if (funcConfig) {
                ...
                const { baseTxData, funcSpecificData, mappingUpdateCandidates } = parseTransactionAndCollectMappingUpdates(
                  tx, 
                  transition,
                  funcConfig, 
                  programConfig
