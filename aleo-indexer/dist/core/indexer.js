 ...
 */
async function getLastIndexedBlock(db, schema, programName) {
 ...
 */
async function updateLastIndexedBlock(db, schema, programName, blockHeight) {
    await db
 ...
 */
async function processProgram(programConfig, allProgramConfigs, // Added to pass to handleProgramMappings
rpcUrl, db, schema) {
    ...
    await handleProgramMappings(mappingUpdateCandidates, // Pass candidates from handleProgramFunctions
 ...
 */
export async function startIndexer(config, db, schema) {
    ...
    const run = async () => {
        ...
        const tasks = config.programs.map((program) => limit(() => 
        // Pass all of `config.programs` to `processProgram`
        processProgram(program, config.programs, config.rpcUrl, db, schema).catch((e) => logger.error({
            service: 'indexer',
