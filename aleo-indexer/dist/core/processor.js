...
function parseTransactionAndCollectMappingUpdates(tx, funcConfig, programConfig) {
    ...
    for (const output of funcConfig.outputs || []) {
        ...
        const parsedData = parseJSONLikeString(transitionOutputs);
        ...
        for (const trigger of funcConfig.triggersMappingUpdates) {
            ...
            if (key !== undefined) { // Ensure key was successfully extracted
                const candidate = {
                    ...
                    key: JS2Leo(String(key), trigger.aleoType.kind === 'primitive' ? trigger.aleoType.type : undefined), // Ensure key is string for RPC calls
    ...
    if (tx.finalize && tx.finalizedAt) {
        for (const finalizeEntry of tx.finalize) {
            ...
            const matchingMappingConfig = programConfig.mappings?.find(m => m.name === finalizeEntry.mapping_id.split('/').pop() // Extract mapping name from mapping_id
            );
...
export async function handleProgramFunctions(programConfig, rpcUrl, db, schema, currentPage) {
    ...
    const processedTransactionsSet = new Set(); // To track transactions already added to the batch
    ...
    while (continueProgramIndexing) {
        ...
        const transactionsCollectedInThisCycle = [];
        for (const funcConfig of programConfig.functions) {
            ...
            while (hasMoreTransactionsForFunction && pagesFetchedInThisFunctionCycle < MAX_PAGES_PER_FUNCTION_CYCLE) {
                ...
                try {
                    ...
                    const response = await callRpc(rpcUrl, "aleoTransactionsForProgram", rpcPayload);
                    // Filter for finalized transactions and ensure they match the program and function
                    transactionsBatch = (response || []).filter(tx => (tx.status === "finalized" || (tx.status === "accepted" && tx.type === "execute")) &&
