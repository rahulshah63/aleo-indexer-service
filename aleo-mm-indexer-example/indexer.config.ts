// example/indexer.config.ts

import { IndexerConfig } from 'aleo-indexer/src/utils/types.js';

const indexerConfig: IndexerConfig = {
  // Aleo RPC URL to connect to. Ensure this is an accessible node.
  ...
  programs: [
    {
      programId: 'token_registry.aleo', // Example: Indexing a token registry program
      
      // Configuration for specific functions within this program
      functions: [
        {
        ...
        },
        {
          ...
          tableName: 'public_transfers',
          inputs: [
            { name: 'token_id', aleoType: { kind: 'primitive', type: 'field' } },
            { name: 'sender', aleoType: { kind: 'primitive', type: 'address' } },
            { name: 'receiver', aleoType: { kind: 'primitive', type: 'address' } },
            { name: 'amount', aleoType: { kind: 'primitive', type: 'u64' } },
          ],
          // NEW: This function triggers updates for the 'token_balances' mapping
          triggersMappingUpdates: [
            {
              mappingName: 'token_balances',
              keySource: 'sender', // 'sender' address from the transfer is a key for token_balances
            },
            {
              mappingName: 'token_balances',
              keySource: 'receiver', // 'receiver' address from the transfer is also a key
            }
            // Note: We don't specify valueSource here because the balance itself is complex state
            // and `getMappingValue` will fetch the current, accurate balance for these addresses.
          ]
        },
