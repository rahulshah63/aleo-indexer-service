...
import { ProgramConfig, ProgramFunctionConfig } from '../config.js';
...
export async function handleProgramEvents(programConfig: ProgramConfig, startPage: number): Promise<number> {
    const BATCH_SIZE = 50;
    ...
    try {
        for (const funcConfig of programConfig.functions) {
            ...
            if (!targetTable) {
                logger.error(`Database table '${funcConfig.tableName}' not found for program '${programConfig.id}' function '${funcConfig.name}'`);
                continue;
            }
            ...
            logger.info({
                ...
                page: currentPage
            }, `Indexed ${programConfig.id}:${funcConfig.name} transactions`);
        }
        ...
        logger.error({ err: error, program: programConfig.id, page: currentPage }, `Failed to index program ${programConfig.id}`);
