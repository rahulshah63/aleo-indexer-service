...
async function processProgram(program: ProgramConfig) { // Expect ProgramConfig type
  ...
  logger.info({ program: program.id, page: lastPage }, "Processing program");
  ...
  if (nextPage > lastPage) {
    ...
    logger.info({ program: program.id, page: nextPage }, "Updated program index state");
  }
}
...
export async function runIndexer() {
  ...
  const run = async () => {
    ...
    const tasks = config.map(program =>
      limit(() => processProgram(program).catch(e => logger.error({ err: e, program: program.id }, "Error processing program")))
    );
