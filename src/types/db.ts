...
import { type PrimaryKeyBuilder  as DrizzlePrimaryKeyBuilder } from "drizzle-orm/pg-core";
import type { NodePgDatabase } from "drizzle-orm/node-postgres";
import type { PgTable, PgTableExtraConfig, TableConfig } from "drizzle-orm/pg-core";
import type { PgliteDatabase } from "drizzle-orm/pglite";
import type { TypeError, Prettify } from "./utils.js";
...
export type Find = <table extends Table>(
  table: table extends { [onchain]: true }
    ? table
    : TypeError<`db.find() can only be used with onchain tables, and '${table["_"]["name"]}' is an offchain table.`>,
  key: Key<table>,
...
>(
  table: table extends { [onchain]: true }
    ? table
    : TypeError<`Indexing functions can only write to onchain tables, and '${table["_"]["name"]}' is an offchain table.`>,
) => {
  /**
   * Create new rows
   *
